...
class Loss:
    def __init__(self, cfg, sim: MPMSimulator):
        ...
        for i in range(len(sim.primitives)):
            primitive = sim.primitives.primitives[i]
    ...
    def get_incremental_iou_kernel(self, f=0):
        # clear and compute grid mass
        self.grid_mass.fill(0)
        self.compute_grid_mass(f)

        # iou calculation
        iou = self.iou_kernel()
        incremental_iou = self.compute_incremental_iou(iou, self._init_iou, self._target_iou)
        return incremental_iou
    ...
    def compute_loss(self, f):
        ...
        incremental_iou = self.compute_incremental_iou(loss_info['iou'], self._init_iou, self._target_iou)
        loss_info['reward'] = r
    ...
    @staticmethod
    def compute_incremental_iou(iou, init_iou, target_iou):
        incremental_iou = max(min((iou - init_iou) / (target_iou - init_iou), 1), 0)
        return incremental_iou
