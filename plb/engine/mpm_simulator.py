...
@ti.data_oriented
class MPMSimulator:
    def __init__(self, cfg, primitives: Primitives = ()):
        ...
        self.C = ti.Matrix.field(dim, dim, dtype=dtype, shape=(max_steps, n_particles),
                                 needs_grad=True)  # affine velocity field
        self.F = ti.Matrix.field(dim, dim, dtype=dtype, shape=(max_steps, n_particles),
                                 needs_grad=True)  # deformation gradient

        self.F_tmp = ti.Matrix.field(dim, dim, dtype=dtype, shape=(n_particles),
                                     needs_grad=True)  # deformation gradient
        ...
        for _ in self.actionable_prim_list:
            self.sdf_dists.append(ti.field(dtype=self.dtype, shape=(self.n_particles,), needs_grad=False))
    ...
    def get_x_grad_kernel(self, f: ti.i32, grad: ti.ext_arr()):
        for i in ti.static(range(0, self.n_particles)):
            for j in ti.static(range(self.dim)):
                grad[i, j] = self.x.grad[f, i][j]

    def get_x_grad(self, f):
        grad = np.zeros((self.n_particles, self.dim), np.float64)
        self.get_x_grad_kernel(f, grad)
        return grad
            ...
            if self.grid_m[I] > 1e-12:  # No need for epsilon here, 1e-10 is to prevent potential numerical problems ..
                ...
                if ti.static(self.n_primitive > 0):
                    for i in ti.static(range(self.n_primitive)):
                        v_out = self.primitives.primitives[i].collide(f, I * self.dx, v_out, self.dt)

                for d in ti.static(range(self.dim)):
                    bound = 3
                    if ti.static(d == 1):
                        bound += self.ground_height / self.dx
    ...
    def g2p(self, f: ti.i32):
        for p in range(0, self.n_particles):  # grid to particle (G2P)
            ...
            self.x[f + 1, p] = ti.max(ti.min(self.x[f, p] + self.dt * self.v[f + 1, p], 1. - 3 * self.dx),
                                      ti.Vector([0., self.ground_height, 0.]))
    ...
    def substep(self, s):
        ...
        for i in range(self.n_primitive):
            self.primitives.primitives[i].forward_kinematics(s)
    ...
    def substep_grad(self, s):
        ...
        for i in range(self.n_primitive - 1, -1, -1):
            self.primitives.primitives[i].forward_kinematics.grad(s)
    ...
    def get_obj_id(self):
        return self.particle_obj_id.to_numpy()
