...
class Cylinder(Primitive):
    def __init__(self, **kwargs):
        ...
        self.h = ti.field(self.dtype, shape=())
        self.r = ti.field(self.dtype, shape=())

    def initialize(self):
        super(Cylinder, self).initialize()
    ...
    def _sdf(self, f, grid_pos):
        ...
        d = ti.abs(ti.Vector([length(ti.Vector([grid_pos[0], grid_pos[2]])), grid_pos[1]])) - ti.Vector(
    ...
    def _normal(self, f, grid_pos):
        ...
        l = length(p)
        d = ti.Vector([l, ti.abs(grid_pos[1])]) - ti.Vector([self.r[None], self.h[None]])
        ...
        n2 = max(d, 0.0) + ti.cast(max(d[0], d[1]) <= 0., self.dtype) * ti.Vector(
...
class Primitives:
    def __init__(self, cfgs, max_timesteps=1024):
        ...
        self.action_dims = [0]
        self.actionable_prim_list = []
        for prim_idx, i in enumerate(outs):
            ...
            if primitive.action_dim > 0:
                self.actionable_prim_list.append(prim_idx)

        self.n = len(self.primitives)

    @property
    def list_actionable(self):
        return self.actionable_prim_list
