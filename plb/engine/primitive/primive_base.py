...
class Primitive:
    ...
    def __init__(self, cfg=None, dim=3, max_timesteps=1024, dtype=ti.f64, **kwargs):
        ...
        self._is_active = ti.field(ti.u8, shape=())
        ...
        ti.root.dense(ti.i, (self.max_timesteps,)).place(self.position, self.position.grad, self.rotation,
                                                         self.rotation.grad,
    ...
    def get_state_kernel(self, f: ti.i32, controller: ti.ext_arr()):
        ...
        for j in ti.static(range(4)):
            controller[j + self.pos_dim] = self.rotation[f][j]
    ...
    def set_state_kernel(self, f: ti.i32, controller: ti.ext_arr()):
        ...
        for j in ti.static(range(4)):
            self.rotation[f][j] = controller[j + self.pos_dim]
    ...
    @property
    def is_active(self):
        return self._is_active[None]

    def set_is_active(self, is_active, position=None, rotation=None):
        assert self.action_dim > 0, "no point of setting is_active if action_dim = 0!"
        self._is_active.fill(int(is_active))
        if not is_active:
            self.position.fill(50.)
            self.action_scale.fill(0.)
            self.xyz_limit.from_numpy(np.array([self.cfg.lower_bound, (50., 50., 50.)]))
        else:
            cfg = self.cfg
            self.action_scale.from_numpy(np.array(cfg.action.scale))
            self.xyz_limit.from_numpy(np.array([cfg.lower_bound, cfg.upper_bound]))
            if position is not None:
                self.position.from_numpy(np.expand_dims(position, axis=0))
            if rotation is not None:
                self.rotation.from_numpy(np.expand_dims(rotation, axis=0))
    ...
    def initialize(self):
        ...
        if has_action := (self.action_dim > 0):
            self.action_scale[None] = cfg.action.scale
        self._is_active[None] = int(has_action)
